...
def compute_angle_and_offset_from_line(line, ref_y, roi_y_start: int, roi_x_start: int):
    ...
    print("x1, y1, x2, y2, x1_full, y1_full, x2_full, y2_full : ", x1, y1, x2, y2, x1_full, y1_full, x2_full, y2_full)
...
def calibrate_all_cameras(config_path='config.yaml', no_config_write=0):
    ...
    for i, dev in enumerate(config['camera']['device_paths']):
        ...
        roi_bright = cv2.add(roi_color, 80)

        selected_line = select_horizontal_line(roi_bright, lines, cam_id=i)
        ...
        cv2.line(rotated_vis, (0, ref_y), (frame.shape[1], ref_y), (0, 255, 0), 1)
        cv2.line(rotated_vis, (0, y_avg), (frame.shape[1], y_avg), (0, 0, 255), 1)
    ...
    if no_config_write == 0:
        with open(f"{config_path}_{time.strftime('%Y%m%d_%H%M%S')}", 'w', encoding='utf-8') as f:
            yaml.dump(config, f, allow_unicode=True)
        print(f"[INFO] {config_path}_{time.strftime('%Y%m%d_%H%M%S')} 보정값 업데이트 완료")
...
def debug_with_img(cam_cnt=6):
    ...
    for p in img_path:
        ...
        selected_line = select_horizontal_line(roi_bright, lines, cam_id=i)
        if selected_line is None:
            print(f"[WARN] 사용자 입력 없음: 기본값 사용")
            angle, offset = 0.0, 0
        else:
            angle, offset = compute_angle_and_offset_from_line(selected_line, ref_y, roi_y_start, roi_x_start)
            print("selected_line : ", selected_line)
            print(": (compute_angle_and_offset_from_line) -> angle, offset : ", angle, offset)
        ...
        y_shift = cv2.warpAffine(rotated, M, (w, h))
